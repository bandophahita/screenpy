...
class TestNarrator:
    ...
    def test_mic_cable_kinked(self):
        ...
        with narrator.mic_cable_kinked():
            ...
            bu_narrations = narrator.backed_up_narrations
            assert len(bu_narrations) == 4
            assert bu_narrations[0][0] == "act"
            assert bu_narrations[1][0] == "scene"
            assert bu_narrations[2][0] == "beat"
            assert bu_narrations[3][0] == "aside"
            assert all(level == 1 for level in map(lambda n: n[-1], bu_narrations))
            MockAdapter.act.assert_not_called()
            MockAdapter.scene.assert_not_called()
            MockAdapter.beat.assert_not_called()
            MockAdapter.act.assert_not_called()
